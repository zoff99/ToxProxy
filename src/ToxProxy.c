...
#define VERSION_PATCH 0
    ...
    static const char global_version_string[] = "2.1.0-ASAN";
#else
    static const char global_version_string[] = "2.1.0";
#endif
...
const char *my_owm_name = "ToxProxy";
...
const int num_entries_name0_list = sizeof(name0_list) / sizeof(name0_list[0]);

const char *name1_list[] = {
...
const int num_entries_name1_list = sizeof(name1_list) / sizeof(name1_list[0]);
...
void bootstrap(Tox *tox)
{
    ...
    DHT_node nodes_bootstrap_nodes[] =
    {
        ...
        {"2a02:587:4c0d:3e8c:2edc:1bfb:ccb6:7e38"   ,    33445, "4B031C96673B6FF123269FF18F2847E1909A8A04642BBECD0189AC8AEEADAF64", {0}},
        ...
        {"2001:b011:4006:9e54::b"                   ,    33445, "3F0A45A268367C1BEA652F258C85F4A66DA76BCAA667A49E770BCC4917AB6A25", {0}},
    ...
    DHT_node nodes_tcp_relays[] =
    {
        ...
        {"2001:b011:4006:9e54::b"                   ,    3389 , "3F0A45A268367C1BEA652F258C85F4A66DA76BCAA667A49E770BCC4917AB6A25", {0}},
        {"tox.initramfs.io"                         ,    33445, "3F0A45A268367C1BEA652F258C85F4A66DA76BCAA667A49E770BCC4917AB6A25", {0}},
        {"2001:b011:4006:9e54::b"                   ,    33445, "3F0A45A268367C1BEA652F258C85F4A66DA76BCAA667A49E770BCC4917AB6A25", {0}},
        {"tox.plastiras.org"                        ,    3389 , "8E8B63299B3D520FB377FE5100E65E3322F7AE5B20A0ACED2981769FC5B43725", {0}},
        {"2605:6400:30:f7f4:c24:f413:e44d:a91f"     ,    3389 , "8E8B63299B3D520FB377FE5100E65E3322F7AE5B20A0ACED2981769FC5B43725", {0}},
        {"tox.plastiras.org"                        ,    33445, "8E8B63299B3D520FB377FE5100E65E3322F7AE5B20A0ACED2981769FC5B43725", {0}},
        {"2605:6400:30:f7f4:c24:f413:e44d:a91f"     ,    33445, "8E8B63299B3D520FB377FE5100E65E3322F7AE5B20A0ACED2981769FC5B43725", {0}},
        ...
        {"95.181.230.108"                           ,    33445, "B5FFECB4E4C26409EBB88DB35793E7B39BFA3BA12AC04C096950CB842E3E130A", {0}},
        {"2a03:c980:db:5d::"                        ,    33445, "B5FFECB4E4C26409EBB88DB35793E7B39BFA3BA12AC04C096950CB842E3E130A", {0}},
        {"95.181.230.108"                           ,    3389 , "B5FFECB4E4C26409EBB88DB35793E7B39BFA3BA12AC04C096950CB842E3E130A", {0}},
        {"2a03:c980:db:5d::"                        ,    3389 , "B5FFECB4E4C26409EBB88DB35793E7B39BFA3BA12AC04C096950CB842E3E130A", {0}},
                                 ...
                                 const uint8_t *UNUSED(group_name), size_t UNUSED(group_name_length), void *UNUSED(user_data))
{
    ...
    int random_index_0 = randombytes_uniform(num_entries_name0_list);
    const char *random_entry_0 = name0_list[random_index_0];
    // printf("Random Entry 0: %s\n", random_entry_0);
    int random_index_1 = randombytes_uniform(num_entries_name1_list);
    const char *random_entry_1 = name1_list[random_index_1];
    // printf("Random Entry 1: %s\n", random_entry_1);
    char *new_peer_name = calloc(1, max_len);
    if (new_peer_name != NULL)
    {
        snprintf(new_peer_name, (max_len - 1), "%s %s", random_entry_0, random_entry_1);
        dbg(LOGLEVEL_DEBUG, "new peername=%s", new_peer_name);

        uint32_t new_grp_num = tox_group_invite_accept(tox, friend_number, invite_data, length,
                                    (const uint8_t *)new_peer_name, strlen(new_peer_name),
                                    NULL, 0,
                                    &error);
...
static void all_groups_peer_name(const Tox *tox)
{
    ...
    for(size_t k=0;k<num_groups;k++)
    {
        ...
        size_t own_name_size = tox_group_self_get_name_size(tox, gnum, &error);
        if (own_name_size == strlen(my_owm_name))
        {
            const int max_len = 1000;
            uint8_t *name = calloc(1, max_len);
            if (name != NULL)
            {
                tox_group_self_get_name(tox, gnum, name, &error);
                if (strncmp((const char *)name, my_owm_name, strlen(my_owm_name)) == 0)
                {
                    int random_index_0 = randombytes_uniform(num_entries_name0_list);
                    const char *random_entry_0 = name0_list[random_index_0];
                    // printf("Random Entry 0: %s\n", random_entry_0);
...
int main(int argc, char *argv[])
{
    ...
    if (sodium_init() == -1) {
        dbg(LOGLEVEL_WARN, "some issue initializing sodium");
    }

    all_groups_peer_name(tox);
...
*/
    ...
    tox_self_set_name(tox, (uint8_t *) my_owm_name, strlen(my_owm_name), NULL);
